...
# Windows amd64
COMMAND="GOOS=windows GOARCH=amd64 go build $BUILD_PARAMETER -o $RELEASE_DIR/${PROJECT_NAME}-windows-amd64.exe"
echo exec: "$COMMAND"
...
# Windows 32-bit
COMMAND="GOOS=windows GOARCH=386 go build $BUILD_PARAMETER -o $RELEASE_DIR/${PROJECT_NAME}-windows-386.exe"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
...
# Windows ARM
COMMAND="GOOS=windows GOARCH=arm64 go build $BUILD_PARAMETER -o $RELEASE_DIR/${PROJECT_NAME}-windows-arm64.exe"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
...
# darwin amd64
COMMAND="GOOS=darwin GOARCH=amd64 go build $BUILD_PARAMETER -o $RELEASE_DIR/${PROJECT_NAME}-darwin-amd64"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
...
# darwin ARM
COMMAND="GOOS=darwin GOARCH=arm64 go build $BUILD_PARAMETER -o $RELEASE_DIR/${PROJECT_NAME}-darwin-arm64"
echo exec: "$COMMAND"
if eval "$COMMAND"; then
